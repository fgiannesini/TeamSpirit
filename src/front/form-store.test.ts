...
import {type SimulationInputs, type SimulationOutputs, type State, useFormStore,} from './form-store.ts';
...
describe('Form store', () => {
    ...
    describe('Generate simulation inputs', () => {
        ...
        test('Should run simulation and store in state', () => {
            ...
            expect(store.$state.simulationOutputs[0]).toStrictEqual(store.$state.simulationOutputs[1])
        });
    });
});
