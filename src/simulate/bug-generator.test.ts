  ...
  describe('Random Bug generator', () => {
    ...
    test.each([
      ...
      (review: number, probability: number) => {
        expect(computeBugProbability(5, 3, 1, review)).toBeCloseTo(probability, 2);
      },
    );
    ...
    test.each([
      ...
      (review: number, probability: number) => {
        expect(computeBugProbability(1, 1, 1, review)).toBeCloseTo(probability, 2);
      },
    );
    ...
    test.each([
      ...
      (review: number, probability: number) => {
        expect(computeBugProbability(5, 3, 5, review)).toBeCloseTo(probability, 2);
      },
    );
    ...
    test('should generate two bugs', () => {
      ...
      expect(bugs).toEqual([todo({ id: 2, name: 'bug-0' }), todo({ id: 3, name: 'bug-1' })]);
    });
    ...
    test.each([
      ...
      (actualPriority, expectedPriority, priorityProbability) => {
        expect(getBugPriority(actualPriority, () => priorityProbability)).toEqual(expectedPriority);
      },
    );
